...
namespace Audio.DMO
{
    ...
    public class MediaObject : IDisposable
    {
        ...
        public DmoMediaType? GetInputType(int inputStream, int inputTypeIndex)
        {
            ...
            {
                if (e.ErrorCode != (int)EDmoHResults.DMO_E_NO_MORE_ITEMS)
                {
                    throw;
                }
            }
        ...
        {
            DmoMediaType mediaType;
            int hresult = (int)mediaObject.GetOutputCurrentType(outputStreamIndex, out mediaType);
            if (hresult == (int)HResult.S_OK)
            {
                // this frees the format (if present)
                // we should therefore come up with a way of marshaling the format
            ...
            {
                if (hresult == (int)EDmoHResults.DMO_E_TYPE_NOT_SET)
                {
        ...
        {
            return SetInputType(inputStreamIndex, mediaType, EDmoSetTypeFlags.DMO_SET_TYPEF_TEST_ONLY);
        }
        ...
        private bool SetInputType(int inputStreamIndex, DmoMediaType mediaType, EDmoSetTypeFlags flags)
        {
            int hResult = (int)mediaObject.SetInputType(inputStreamIndex, ref mediaType, flags);
            if (hResult != (int)HResult.S_OK)
            {
                if (hResult == (int)EDmoHResults.DMO_E_INVALIDSTREAMINDEX)
                {
                    throw new ArgumentException("Invalid stream index");
                }
                if (hResult == (int)EDmoHResults.DMO_E_TYPE_NOT_ACCEPTED)
                {
        ...
        {
            if(!SetInputType(inputStreamIndex,mediaType, EDmoSetTypeFlags.None))
            {
        ...
        public void SetInputWaveFormat(int inputStreamIndex, WaveFormat waveFormat)
        {
            ...
            bool set = SetInputType(inputStreamIndex, mediaType, EDmoSetTypeFlags.None);
            DmoInterop.MoFreeMediaType(ref mediaType);
            if (!set)
            {
                throw new ArgumentException("Media Type not supported");
        ...
        {
            DmoMediaType mediaType = CreateDmoMediaTypeForWaveFormat(waveFormat);
            bool supported = SetInputType(inputStreamIndex, mediaType, EDmoSetTypeFlags.DMO_SET_TYPEF_TEST_ONLY);
            DmoInterop.MoFreeMediaType(ref mediaType);
            return supported;
        }
        ...
        {
            return SetOutputType(outputStreamIndex, mediaType, EDmoSetTypeFlags.DMO_SET_TYPEF_TEST_ONLY);
        }
        ...
        {
            DmoMediaType mediaType = CreateDmoMediaTypeForWaveFormat(waveFormat);
            bool supported = SetOutputType(outputStreamIndex, mediaType, EDmoSetTypeFlags.DMO_SET_TYPEF_TEST_ONLY);
            DmoInterop.MoFreeMediaType(ref mediaType);
            return supported;
        }
        ...
        /// </summary>
        private bool SetOutputType(int outputStreamIndex, DmoMediaType mediaType, EDmoSetTypeFlags flags)
        {
            int hresult = (int)mediaObject.SetOutputType(outputStreamIndex, ref mediaType, flags);
            if (hresult == (int)EDmoHResults.DMO_E_TYPE_NOT_ACCEPTED)
            {
                return false;
            }
            else if (hresult == (int)HResult.S_OK)
            {
                return true;
            }
            else
        ...
        {
            if (!SetOutputType(outputStreamIndex, mediaType, EDmoSetTypeFlags.None))
            {
                throw new ArgumentException("Media Type not supported");
            }
        }
        ...
        {
            DmoMediaType mediaType = CreateDmoMediaTypeForWaveFormat(waveFormat);
            bool succeeded = SetOutputType(outputStreamIndex, mediaType, EDmoSetTypeFlags.None);
            DmoInterop.MoFreeMediaType(ref mediaType);
            if (!succeeded)
            {
                throw new ArgumentException("Media Type not supported");
        ...
        public void ProcessInput(int inputStreamIndex, IMediaBuffer mediaBuffer, EDmoInputDataBufferFlags flags,
        ...
        public void ProcessOutput(EDmoProcessOutputFlags flags, int outputBufferCount, DmoOutputDataBuffer[] outputBuffers)
        {
            int reserved;
        ...
        {
            EDmoInputStatusFlags flags;
            int hresult = mediaObject.GetInputStatus(inputStreamIndex, out flags);
            Marshal.ThrowExceptionForHR(hresult);
            return (flags & EDmoInputStatusFlags.DMO_INPUT_STATUSF_ACCEPT_DATA) == EDmoInputStatusFlags.DMO_INPUT_STATUSF_ACCEPT_DATA;
        }
