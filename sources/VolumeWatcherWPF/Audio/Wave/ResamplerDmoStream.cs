        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            while (outputBytesProvided < count)
            {
                if (resampler.MediaObject.IsAcceptingData(0))
                {
                    ...
                    resampler.MediaObject.ProcessInput(0, inputMediaBuffer, EDmoInputDataBufferFlags.None, 0, 0);

                    outputBuffer.MediaBuffer.SetLength(0);
                    outputBuffer.StatusFlags = EDmoOutputDataBufferFlags.None;

                    // 4. Now ask the DMO for some output data
                    resampler.MediaObject.ProcessOutput(EDmoProcessOutputFlags.None, 1, new DmoOutputDataBuffer[] { outputBuffer });
